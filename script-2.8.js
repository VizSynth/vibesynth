...
function compileShaders() {
  ...
  const fragLayer = `
    ...
    void main() {
      ...
      // Apply opacity to the final blend
      vec3 finalColor = mix(base.rgb, result, u_opacity);
...
function createFinalOutputNode() {
  ...
  console.log('🖥️ Created permanent Canvas Display node');
...
function performAutoLayout() {
  ...
  // Group nodes by category, with special handling for Canvas Display
  const nodesByCategory = {
    input: nodes.filter(n => n.category === 'input'),
    source: nodes.filter(n => n.category === 'source'), 
    effect: nodes.filter(n => n.category === 'effect'),
    compositing: nodes.filter(n => n.category === 'compositing'),
    system: nodes.filter(n => n.category === 'system' && n.name !== 'Canvas Display')
  };
  
  // Find Canvas Display node for special positioning
  const canvasDisplayNode = nodes.find(n => n.name === 'Canvas Display');
  ...
  // Position Canvas Display node in bottom right area
  if (canvasDisplayNode) {
    // Calculate bottom right position based on graph content
    const rightmostX = Math.max(
      LAYOUT_CONFIG.layers.system.x + 200, // Default right position
      ...nodes.filter(n => n !== canvasDisplayNode).map(n => n.x + 200) // Account for existing nodes
    );
    
    // Find the bottom position based on existing nodes
    const bottomY = Math.max(
      LAYOUT_CONFIG.padding.y + 300, // Minimum bottom position
      ...nodes.filter(n => n !== canvasDisplayNode).map(n => n.y + 150) // Below other nodes
    );
    
    // Position Canvas Display consistently in bottom right
    const canvasX = rightmostX + 50;
    const canvasY = bottomY + 50;
    
    console.log(`🖥️ Positioning Canvas Display at (${canvasX}, ${canvasY})`);
    animateNodeToPosition(canvasDisplayNode, canvasX, canvasY);
  }
...
function createNode(type, x = 100, y = 100) {
  ...
  if (autoLayoutEnabled && nodes.length > 1) { // Don't auto-layout if only Canvas Display exists
...
function validateDataModelConsistency() {
  ...
  // Skip dropdown validation since Canvas Display logic is different
  // The dropdown now controls what feeds Canvas Display, not what Canvas Display is
...
function showNodeProperties(node) {
  ...
  if (!node) {
    panel.innerHTML = '<div class="properties-empty">Select a node to view properties</div>';
    return;
  }
  ...
  if (node.type === 'AudioInput') {
    html += `<div class="property-field">
      <label class="property-label">Audio Access</label>
    ...
    html += `<div class="property-field">
      <label class="property-label">Camera Access</label>
    ...
    html += `<div class="property-field">
      <label class="property-label">Current Output</label>
  ...
  Object.entries(node.params).forEach(([key, value]) => {
    ...
    html += `<div class="property-field">
      <label class="property-label">${labelText}</label>`;
    ...
    if (typeof value === 'number') {
      ...
      if (isSlider) {
        ...
        html += `<div class="dual-input-container">
          <input type="range" class="property-slider" data-param="${key}" 
                 value="${value}" min="${min}" max="${max}" step="${step}">
          <input type="number" class="property-number" data-param="${key}" 
                 value="${value}" min="${min}" max="${max}" step="${step}">
        </div>`;
      } else {
        html += `<input type="number" class="property-input" data-param="${key}" value="${value}">`;
      }
    } else if (typeof value === 'boolean') {
      html += `<input type="checkbox" class="property-input" data-param="${key}" ${value ? 'checked' : ''}>`;
    } else if (typeof value === 'string') {
      const constraints = getParameterConstraints(key);
      
      if (key === 'color' || (constraints && constraints.type === 'color')) {
        html += `<input type="color" class="property-input" data-param="${key}" value="${value}">`;
      } else if (constraints && constraints.values) {
        // Generate dropdown from constraint values
        html += `<select class="property-input" data-param="${key}">
        ...
        html += `<input type="text" class="property-input" data-param="${key}" value="${value}">`;
      }
    }
...
function renderFinalOutput() {
  const finalOutputNode = nodes.find(n => n.type === 'FinalOutput');
  if (!finalOutputNode || !finalOutputNode.inputs[0] || !finalOutputNode.inputs[0].texture) {
    return;
  }
  ...
  Object.entries(node.params).forEach(([key, value]) => {
    ...
    if (loc) {
      if (typeof value === 'number') {
        ...
        gl.uniform1i(loc, modes[value] || 0);
  ...
  node.inputs.forEach((inputNode, index) => {
    if (inputNode && inputNode.texture && !inputNode.deleted) {
      ...
      if (!gl.isTexture(inputNode.texture)) {
        console.warn(`Input node ${inputNode.name} has invalid texture - skipping texture binding`);
        return;
      }
      
      const uniformName = index === 0 ? "u_texture" : `u_texture${index + 1}`;
      const loc = gl.getUniformLocation(program, uniformName);
      if (loc) {
...
function updateMainOutputDropdown() {
  ...
  // Find the Canvas Display node (formerly Final Output)
  const canvasDisplayNode = nodes.find(n => n.type === 'FinalOutput');
  if (!canvasDisplayNode) return; // Guard against missing node
  
  // Always use Canvas Display as the main output (it shows whatever is connected to it)
  mainOutputNode = canvasDisplayNode;
  
  select.innerHTML = '<option value="">None</option>';
  
  // Show what's currently connected to Canvas Display
  ...
  select.onchange = (e) => {
    ...
    if (e.target.value === '') {
      // Disconnect from Canvas Display
      canvasDisplayNode.inputs[0] = null;
    } else {
      // Connect selected node to Canvas Display
    ...
    updateConnections();
    updateCanvasDisplay();
    saveState('Change Canvas Display Input');
  };
...
function updateCanvasDisplay() {
  const displaySpan = document.getElementById('current-display-node');
  if (!displaySpan) return; // Guard against missing DOM element
  
  const canvasDisplayNode = nodes.find(n => n.type === 'FinalOutput');
  if (!canvasDisplayNode) {
    displaySpan.textContent = 'Canvas Display not found';
    displaySpan.style.color = 'var(--text-muted)';
    return;
  }
  
  // Always show what's connected to Canvas Display
  const connectedNode = canvasDisplayNode.inputs && canvasDisplayNode.inputs[0];
  if (connectedNode && connectedNode.name) {
    displaySpan.textContent = connectedNode.name;
    displaySpan.style.color = 'var(--accent-primary)';
  } else {
    displaySpan.textContent = 'Nothing connected';
    displaySpan.style.color = 'var(--text-muted)';
  }
}
  ...
  try {
    ...
    if (finalOutputNode && projectData.finalOutputInput) {
      ...
      if (connectedNode) {
        ...
        console.log('✅ Restored Canvas Display connection to:', connectedNode.name);
      }
    }
...
function showUpdateNotification() {
  ...
  overlay.innerHTML = `
    <div class="update-content">
      ...
      <p>A new version of Visual Synthesizer is available. Your current project will be automatically restored after the update.</p>
      <div class="update-actions">
        <button id="update-now-btn" class="create-btn">
  ...
  // Add event listeners
  document.getElementById('update-now-btn').addEventListener('click', performUpdate);
